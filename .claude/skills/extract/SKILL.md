...
5. **Present the plan via `ExitPlanMode`**, formatted per `.claude/skills/extract/references/plan-template.md`. If the session isn't already in plan mode, call `EnterPlanMode` first. The plan must list, per proposed note: filename, title, type + one-sentence rationale, `## Related` links (mark `(stub)` where applicable), writer uncertainty, and verifier results.
...
7. **Write files** to `things/` after approval. If the user overruled a verifier flag, record the disagreement in the note body under `## Related` (e.g. `Verifier flagged: <item> — user overruled because <reason>.`).
