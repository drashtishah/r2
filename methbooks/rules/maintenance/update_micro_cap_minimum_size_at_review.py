...
def update_micro_cap_minimum_size_at_review(df: pl.DataFrame) -> pl.DataFrame:
    ...
    # Business assert: threshold must be strictly positive after update.
    min_size = float(out["micro_cap_min_size_usd"].min())
    assert min_size > 0, (
        f"micro_cap_min_size_usd is non-positive after review update: {min_size}"
    )
    # Business assert: when coverage was in band, threshold was left unchanged.
    if at_prior_rank.height > 0:
        coverage = float(at_prior_rank["cumulative_ff_mcap_coverage_pct"].item(0))
        if LOWER_BAND_PCT <= coverage <= UPPER_BAND_PCT:
            before = float(df["micro_cap_min_size_usd"].min())
            after = min_size
            assert abs(after - before) < 1e-6, (
                f"micro_cap_min_size_usd changed despite coverage {coverage}"
